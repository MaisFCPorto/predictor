...
var ShopSupabaseService = class {
  ...
  async updateProductStock(productId, quantity) {
    const { data: product, error: fetchError } = await this.client.from("shop_products").select("stock").eq("id", productId).single();
    if (fetchError) throw fetchError;
    const newStock = Math.max(0, product.stock - quantity);
    const { data, error } = await this.client.from("shop_products").update({ stock: newStock }).eq("id", productId).select().single();
    if (error) throw error;
    return data;
  }
  ...
  async listProducts() {
    const { data, error } = await this.client.from("shop_products").select("*").order("created_at", { ascending: false });
    if (error) throw error;
    return data;
  }
};
...
// src/routes/shop/products.ts
var products = new Hono2();
products.options("*", (c) => c.body(null, 204));
products.get("/", async (c) => {
  try {
    const shopService = new ShopSupabaseService(c.env);
    const products2 = await shopService.listProducts();
...
// .wrangler/tmp/bundle-dcIz62/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-dcIz62/middleware-loader.entry.ts
