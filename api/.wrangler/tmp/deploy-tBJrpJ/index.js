    ...
    await DB.prepare(
      `
        ...
        WHERE user_id = ?
          AND fixture_id = ?
      `
    ).bind(pts, p.user_id, p.fixture_id).run();
  }
}
...
async function recomputePointsForFixture2(db, fixtureId) {
  const fx = await db.prepare(`
...
__name(recomputePointsForFixture2, "recomputePointsForFixture");
...
app.patch("/api/admin/fixtures/:id", async (c) => {
  ...
  if (touchesScore) {
    await recomputePointsForFixture2(c.env.DB, id);
  }
  return c.json({ ok: true });
});
