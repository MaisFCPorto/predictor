...
function target(req: NextRequest) {
  ...
  return `${UPSTREAM}/api/fixtures/finished${qs}`;
}
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (!r.ok) {
      ...
      return NextResponse.json({ where:'/api/fixtures/finished', upstream:url, status:r.status, snippet:snip }, { status:r.status });
    }
    return new NextResponse(buf, { status:r.status, headers:h });
  } catch (e:any) {
    return NextResponse.json({ where:'/api/fixtures/finished', upstream:url, error:String(e?.message || e) }, { status: 500 });
  }
}
