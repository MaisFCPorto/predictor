...
adminFixtureScorers.put("/:id/scorers", async (c) => {
  ...
  await c.env.DB.prepare(`DELETE FROM fixture_scorers WHERE fixture_id = ?`).bind(fixtureId).run();
...
async function recomputePointsForFixture(db, fixtureId) {
  ...
  for (const p of results ?? []) {
    ...
    await db.prepare(`
        ...
        WHERE user_id = ?
          AND fixture_id = ?
      `).bind(s.points, p.user_id, p.fixture_id).run();
  }
}
...
app.post("/api/predictions", async (c) => {
  try {
    ...
    const scorerId = typeof scorer_player_id === "string" && scorer_player_id.trim() ? scorer_player_id.trim() : null;
...
app.get("/api/predictions", async (c) => {
  try {
    ...
    const safe = (results ?? []).map((r) => ({
      ...
      points: r.points,
      // pode ser null se ainda não foi calculado
...
// .wrangler/tmp/bundle-iTLSNd/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-iTLSNd/middleware-loader.entry.ts
