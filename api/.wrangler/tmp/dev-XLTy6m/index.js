...
// src/app-settings.ts
async function getRequiredAppSetting(db, key) {
...
__name(getRequiredAppSetting, "getRequiredAppSetting");
function getActiveSeasonId(db) {
  return getRequiredAppSetting(db, "active_season_id");
}
__name(getActiveSeasonId, "getActiveSeasonId");
function getActiveMatchdayId(db) {
  return getRequiredAppSetting(db, "active_matchday_id");
}
__name(getActiveMatchdayId, "getActiveMatchdayId");
...
winners.get("/monthly", async (c) => {
  ...
  const activeSeasonId = await getActiveSeasonId(c.env.DB);
  const params = [activeSeasonId, ym];
  ...
  try {
    ...
    const rowsRes = await db.prepare(
      `
        ...
        `
    ).bind(activeSeasonId, leagueId).all();
...
app.get("/api/fixtures/finished", async (c) => {
  ...
  const { results } = await c.env.DB.prepare(
    `
      ...
      LIMIT ? OFFSET ?
    `
  ).bind(activeSeasonId, limit, offset).all();
  return c.json(results ?? []);
});
app.get("/api/fixtures/closed", async (c) => {
  ...
  const { results } = await c.env.DB.prepare(
    `
      ...
      LIMIT ? OFFSET ?
    `
  ).bind(activeSeasonId, lockStr, limit, offset).all();
  const rows = results ?? [];
...
app.post("/api/admin/fixtures", async (c) => {
  ...
  try {
    ...
    const matchday_id = b.matchday_id != null && String(b.matchday_id).trim() !== "" ? String(b.matchday_id).trim() : await getActiveMatchdayId(c.env.DB);
    await run2(
      c.env.DB,
...
app.get("/api/users/:id/last-points", async (c) => {
  ...
  const last = await db.prepare(
    `
      ...
      LIMIT 1
    `
  ).bind(userId, activeSeasonId).first();
  if (!last) {
    return c.json(null);
  }
...
// .wrangler/tmp/bundle-2qK0ij/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-2qK0ij/middleware-loader.entry.ts
