...
winners.get("/", async (c) => {
  ...
  const { results: predRows } = await c.env.DB.prepare(
    `
      SELECT
        p.user_id,
        p.fixture_id,
  ...
  const fixtureInfoById = /* @__PURE__ */ new Map();
  const byFixture = {};
  for (const p of preds) {
  ...
  const winnersList = Object.keys(byFixture).map((fixtureId) => {
    const scores = Object.values(byFixture[fixtureId] ?? {});
    scores.sort(cmpRanking2);
    const top = scores[0];
    ...
    const da = new Date(a.kickoff_at).getTime();
    const db = new Date(b.kickoff_at).getTime();
    return db - da;
  });
...
winners.get("/monthly", async (c) => {
  ...
  const top3 = Object.values(score).sort(cmpRanking2).slice(0, 3).map((r) => ({
    ym,
    user_id: r.user_id,
    name: r.name,
    email: r.email
...
function scorerBonusForPosition3(pos) {
  if (!pos) return 0;
  const key = pos.toUpperCase();
  return SCORER_BONUS_BY_POS3[key] ?? 0;
}
__name(scorerBonusForPosition3, "scorerBonusForPosition");
async function recomputePointsForFixture(DB, fixtureId) {
  ...
  for (const r of scorerRows ?? []) {
    const bonus = scorerBonusForPosition3(r.position);
    if (bonus) {
...
// src/services/supabase-shop.ts
var ShopSupabaseService = class {
  static {
    __name(this, "ShopSupabaseService");
  }
  client;
  constructor(env) {
  ...
  async createOrder(orderData) {
    const { data, error } = await this.client.from("shop_orders").insert(orderData).select().single();
    if (error) throw error;
    return data;
  }
  async createOrderItems(items) {
    const { data, error } = await this.client.from("shop_order_items").insert(items).select();
    if (error) throw error;
    return data;
  }
  async updateProductStock(productId, quantity) {
    const { data, error } = await this.client.from("shop_products").update({ stock: this.client.sql`stock - ${quantity}` }).eq("id", productId).select().single();
    if (error) throw error;
    return data;
  }
  async createPayment(paymentData) {
    const { data, error } = await this.client.from("shop_payments").insert(paymentData).select().single();
    if (error) throw error;
    return data;
  }
  async getPayment(id) {
    const { data, error } = await this.client.from("shop_payments").select("*").eq("id", id).single();
    if (error) throw error;
    return data;
  }
  async updatePaymentStatus(id, status, paidAt) {
    ...
    const { data, error } = await this.client.from("shop_payments").update(updateData).eq("id", id).select().single();
    if (error) throw error;
    return data;
  }
  async updateOrderStatus(orderId, status) {
    const { data, error } = await this.client.from("shop_orders").update({ status }).eq("id", orderId).select().single();
    if (error) throw error;
    return data;
  }
  async getOrder(id) {
    const { data, error } = await this.client.from("shop_orders").select(`
        *,
        shop_order_items (
          *,
  ...
  async getPaymentByEuPagoId(eupagoPaymentId) {
    const { data, error } = await this.client.from("shop_payments").select("*").eq("eupago_payment_id", eupagoPaymentId).single();
    if (error) throw error;
    return data;
  }
};

// src/routes/shop/orders.ts
...
orders.get("/", requireAdminKey, async (c) => {
  try {
    const shopService = new ShopSupabaseService(c.env);
    const orders2 = await shopService.listOrders();
...
// src/services/eupago.ts
var EuPagoService = class {
  static {
    __name(this, "EuPagoService");
  }
  apiKey;
  baseUrl;
  constructor(env) {
  ...
  async createMultibanco(data) {
    return this.request("/payments/multibanco", data);
  }
  async createMBWay(data) {
    return this.request("/payments/mbway", data);
  }
  async getPaymentStatus(paymentId) {
    return this.request("/payments/status", { id: paymentId });
  }
};

// src/routes/shop/payments.ts
...
payments.post("/", async (c) => {
  ...
  const { results: users } = await c.env.DB.prepare("SELECT * FROM users WHERE id = ?").bind(order.user_id).all();
  const user = users[0];
  if (!user) {
  ...
  try {
    ...
    await shopService.createPayment({
      ...
      entity: method === "multibanco" ? eupagoResponse.payment.entity : void 0,
      reference: eupagoResponse.payment.reference,
...
// .wrangler/tmp/bundle-SV70YI/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-SV70YI/middleware-loader.entry.ts
